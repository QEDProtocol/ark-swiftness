...
pub const KECCAK_RATIO: usize = 2048;
pub const KECCAK_ROW_RATIO: usize = 32768;
pub const LAYOUT_CODE: Felt =
    Felt::from_hex_unchecked("0x737461726b6e65745f776974685f6b656363616b");
...
impl<F: SimpleField + PoseidonHash> LayoutTrait<F> for Layout {
    ...
    fn eval_composition_polynomial(
        ...
        let n_pedersen_hash_copies = n_steps.field_div(&F::from_constant(
            (PEDERSEN_BUILTIN_RATIO * PEDERSEN_BUILTIN_REPETITIONS) as u128,
        ));
        ...
        let n_keccak_component_copies =
            n_steps.field_div(&F::from_constant((DILUTED_N_BITS * KECCAK_RATIO) as u128));
        // assert!(n_keccak_component_copies < u128::MAX.into());
        n_keccak_component_copies.assert_lt(&F::from_constant(u128::MAX));
        let n_keccak_periodic_columns_copies =
            F::from_constant(2048_u128) * n_keccak_component_copies;
        let keccak_point = point.powers_felt(&n_keccak_periodic_columns_copies);
        let keccak_keccak_keccak_round_key0 = eval_keccak_round_key0(keccak_point.clone());
        let keccak_keccak_keccak_round_key1 = eval_keccak_round_key1(keccak_point.clone());
        let keccak_keccak_keccak_round_key3 = eval_keccak_round_key3(keccak_point.clone());
        let keccak_keccak_keccak_round_key7 = eval_keccak_round_key7(keccak_point.clone());
        let keccak_keccak_keccak_round_key15 = eval_keccak_round_key15(keccak_point.clone());
        let keccak_keccak_keccak_round_key31 = eval_keccak_round_key31(keccak_point.clone());
        let keccak_keccak_keccak_round_key63 = eval_keccak_round_key63(keccak_point);

        let n_poseidon_copies = n_steps.field_div(&F::from_constant(POSEIDON_RATIO as u128));
        // assert!(n_pedersen_hash_copies < u128::MAX.into());
        n_pedersen_hash_copies.assert_lt(&F::from_constant(u128::MAX));
        ...
        let global_values = GlobalValues {
            ...
            offset_size: F::from_constant(0x10000 as u128), // 2**16
            half_offset_size: F::from_constant(0x8000 as u128), // 2**15
