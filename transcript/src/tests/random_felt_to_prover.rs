...
fn test_random_felts_to_prover_0() {
    ...
    assert!(
        *transcript.counter() == StarkArkConvert::from_stark_felt(Felt::from_hex_unchecked("0x1"))
    );
}
...
fn test_random_felts_to_prover_1() {
    ...
    assert!(
        *transcript.counter() == StarkArkConvert::from_stark_felt(Felt::from_hex_unchecked("0x2"))
    );
}
