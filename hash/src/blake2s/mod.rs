...
impl<F: PrimeField + SimpleField, ConstraintF: PrimeField + SimpleField> Blake2sHash for NonNativeFieldVar<F, ConstraintF> {
    fn hash(data: &[<Self as SimpleField>::ByteType]) -> Vec<<Self as SimpleField>::ByteType> {
        let mut input_bits = vec![];
        for input_byte in data.iter() {
            input_bits.extend(ToBitsGadget::<ConstraintF>::to_bits_le(input_byte).unwrap());
        }
